...
class LLMConfig(BaseModel):
    """LLM設定（デフォルトはOllama、HFバックエンドも選択可）"""
    ...
    backend: Literal["ollama", "hf"] = Field(
        default=os.getenv("LLM_BACKEND", "ollama")
    )
    hf_model_name: Optional[str] = Field(
        default=os.getenv("HF_MODEL_NAME", None)
    )
    lora_path: Optional[str] = Field(
        default=os.getenv("LORA_PATH", None)
    )
    load_in_4bit: bool = Field(
        default=os.getenv("LOAD_IN_4BIT", "true").lower() == "true"
    )
    bnb_4bit_compute_dtype: str = Field(
        default=os.getenv("B2_4BIT_COMPUTE_DTYPE", os.getenv("B2_4BIT_COMPUTE_DTYPE", "bfloat16"))
    )
    bnb_4bit_quant_type: str = Field(
        default=os.getenv("B2_4BIT_QUANT_TYPE", os.getenv("B2_4BIT_QUANT_TYPE", "nf4"))
    )
